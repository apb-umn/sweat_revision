...
prices_init = np.array([p, rc, pkap, ome, varpi, theta])
del p, rc, pkap, ome, varpi, theta
...
def target(prices):
    ...
    # dist = np.sqrt(moms[0]**2.0 + moms[1]**2.0 + 0.1*moms[8]**2.0)

    dist = np.sqrt(moms[0]**2.0 + moms[1]**2.0 + 0.1*moms[8]**2.0 +
                   (moms[4]/s_emp_share - 1.)**2.0 +\
                   (moms[5]/pure_sweat_share - 1.)**2.0 +\
                   (moms[9]/xc_share - 1.)**2.0 ) 
    ...
    f.writelines(str(dist) + ', ' +str(p_) + ', ' + str(rc_) + ', ' + str(pkap_) + ', ' + str(kapbar)  + ', ' +str(ome_) + ', ' + str(theta_) + ', ' + str(varpi_) + ', ' +   str(moms[0]) + ', ' + str(moms[1]) + ', ' + str(moms[2]) + ', ' + str(moms[4]) + ', ' + str(moms[5]) + ', ' + str(moms[7]) + ', ' + str(moms[8]) + ', ' + str(moms[9])  +  '\n')
...
if __name__ == '__main__':
    ...
    # p, rc, pkap, kapbar, ome, theta, varapi
    
    f = open(nd_log_file, 'a')
    f.writelines('dist, p, rc, pkap, kapbar, ome, theta, varpi, mom0, mom1, mom2, mom4, mom5, mom7, mom8, mom9\n')        
    f.close()
