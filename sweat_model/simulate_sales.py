...
if __name__ == '__main__':
    ...
    from parameters_sales_benchmark import *
    # from parameters_sales_taunonly import *    
    ...
    generate_shock(prob = np.array([[1. - pi_sell, pi_sell], [1. - pi_sell, pi_sell]]),
                   num_agent = num_total_pop,
                   num_time = sim_time,
                   buffer_time = buffer_time,
    ...
    # set prices
    econ.set_prices(p = p, rc = rc, pkap = pkap, kapbar = kapbar)
    ...
    econ.save_result_csv(dir_path_save = save_path) # save simulation result under ./save_data/ by default
        ...
        # create a directory to save files if it does not exist.
    # dirty way to create a sub directory.
    save_path = save_path + 'tables/'
    pathlib.Path(save_path).mkdir(exist_ok=True)
